...
# seperate data into two seasons

ds.low.yield.IP <- subset(IPwithyield, season == DS, yield.level == "low")
ds.medium.yield.IP <- subset(IPwithyield, season == DS, yield.level == "medium")
ds.high.yield.IP <- subset(IPwithyield, season == DS, yield.level == "high")


# = group = #
ws.low.yield.IP <- subset(IPwithyield, season == WS, yield.level == "low")
ws.medium.yield.IP <- subset(IPwithyield, season == WS, yield.level == "medium")
ws.high.yield.IP <- subset(IPwithyield, season == WS, yield.level == "high")

ds.level.yield.IP.list <- list(ds.low.yield.IP, ds.medium.yield.IP, ds.high.yield.IP)

ws.level.yield.IP.list <- list(ws.low.yield.IP, ws.medium.yield.IP, ws.high.yield.IP)

# Dry Season Yield ====================================================================
source("~/Documents/Github/network.project/chapter3/R/functions/function_cooc_table.R")
ds.level.yield.IP.cor.mat <- list()

for (i in 1:length(level.yield.IP.list)) {
  
  # select out the country and season varible
  temp <- ds.level.yield.IP.list[[i]][!names(ds.level.yield.IP.list[[i]]) %in% c("index", "country", "year", "season", "SNL", "AW", "mean.yield", "yield.level")]
...
ds.level.yield.IP.net <- list()

for (i in 1:length(ds.level.yield.IP.cor.mat)) {
  
  # keep the correlation coefficient at p.value < 0.05
  cut.table <- ds.level.yield.IP.cor.mat[[i]] %>% filter(p.value < 0.05)
  
  # construct the netwotk object 
  ds.level.yield.IP.nyet[[i]] <- plot_network(cut.table)
  
}

plot(ds.level.yield.IP.net[[1]])
# plot(level.yield.IP.net[[2]])
# plot(level.yield.IP.net[[3]])
ds.pair.yield.all <- ds.level.yield.IP.cor.mat[[1]] %>% dplyr::select(var1, var2) %>% mutate(pair = paste(var1, var2, sep = "_"))

ds.pair.yield.low <- level.yield.IP.cor.mat[[1]] %>% filter(p.value < 0.05) %>% dplyr::select(var1, var2) %>% mutate(pair = paste(var1, var2, sep = "_"))

ds.pair.yield.medium <- level.yield.IP.cor.mat[[2]] %>% filter(p.value < 0.05) %>% dplyr::select(var1, var2) %>% mutate(pair = paste(var1, var2, sep = "_"))

ds.pair.yield.high <- level.yield.IP.cor.mat[[3]] %>% filter(p.value < 0.05) %>% dplyr::select(var1, var2) %>% mutate(pair = paste(var1, var2, sep = "_"))
...
#node.df <- node.df %>% select(group, everything())

#new.reform <- node.df %>% melt(id.vars = c("var", "group"), 
#                               variable.name = "Node_Measure",
#                               value.name = "Score") 

#ew.reform$group <- factor(new.reform$group, levels = c("low", "medium", "high"))
#ggplot(new.reform, aes(x = group, y = Score)) + geom_boxplot() + facet_wrap( ~ Node_Measure, scales = 'free', ncol = 4)
...
# reorder the colummn by moving column named country_season into the first
net.df <- net.df %>% select(group, everything())
